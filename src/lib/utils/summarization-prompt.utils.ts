...
- "actionItems": Next steps — each has "description" (what), "owner" (who), and "dueDate" (when this needs to happen). Populate "dueDate" with ANY timing reference mentioned for that specific action item, even if relative, informal, or approximate (e.g. "Thursday", "next week", "before the fall buying season", "in two weeks") — copy the phrasing used in the transcript verbatim rather than converting it to a calendar date or inventing one. Only use null if no timing was mentioned at all for that item — do not default to null just because the timing is informal.
