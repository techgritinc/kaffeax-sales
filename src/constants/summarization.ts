...
5. **Compose the output.** Distribute the inventoried facts across the output fields. No fact from steps 2 or 3 may be discarded. Write the narrative by covering, in this order of ideas within the same flowing 1-2 paragraph prose, why the meeting happened, then the most important outcomes, then what remains unresolved — never as separate paragraphs or labeled sections.
6. **Cross-check for duplication before finalizing.** Compare your drafted "whatWasCovered", "whatWasDecided", and "actionItems" against each other. If the same fact appears in more than one, keep it only in the single correct field — a forward-looking commitment with an owner belongs in "actionItems" only; a procedural agreement that is not itself an assignable task belongs in "whatWasDecided" only; everything else discussed belongs in "whatWasCovered" only — and remove it from the rest before returning the JSON.`;
...
export const OUTPUT_SCHEMA_EXAMPLE = `{
  ...
  "narrative": "1-2 paragraph prose synthesis, no headings or labels, that makes clear why the meeting happened, the most important outcomes, and what remains unresolved — every guardrail, constraint, timeline, budget, commitment, and concern must still appear here or in the structured fields below",
...
- "narrative": A faithful prose synthesis of the meeting in the same 1-2 paragraph format used today — never a longer, multi-paragraph retelling, and never with headings, labels, or bullet points added to mark structure. Synthesize the call; do not narrate it turn-by-turn. Within that flowing prose, make clear: why the meeting happened, the most important outcomes from the call, and what remains unresolved at its end — in that order of ideas, without separating them into distinct paragraphs or labeled sections. Every guardrail, constraint, timeline, budget reference, commitment, objection, and concern raised by either party must still be preserved, here or in the structured fields below — brevity must never cause information to be dropped. Do not smooth over specifics — retain exact figures, names, and terms. Management reads this to know precisely what happened and what was agreed; nothing consequential may be omitted or softened.
...
- "whatWasCovered": Topics discussed during the meeting that are neither a decision reached nor a future commitment — e.g. context exchanged, current state described, or options explored without resolution. Empty array if none.
- "whatWasDecided": Every decision made or agreement reached that is not itself a discrete, assignable follow-up task — an agreement about how the parties will proceed. Capture exact terms — avoid vague summaries like "agreed to follow up" when the transcript says "agreed to a 30-minute demo on Thursday next week".
- "actionItems": Next steps — each has "description" (what), "owner" (who), and "dueDate" (when this needs to happen). Populate "dueDate" with ANY timing reference mentioned for that specific action item, even if relative, informal, or approximate (e.g. "Thursday", "next week", "before the fall buying season") — copy the phrasing verbatim. Only use null if no timing was mentioned at all for that item. Any forward-looking commitment with an owner belongs here, and here only.

"whatWasCovered", "whatWasDecided", and "actionItems" MUST partition the call's facts with zero overlap — no fact, decision, or commitment may appear — verbatim or in close paraphrase — in more than one of these three fields. When a fact could fit more than one, use this precedence (highest wins): (1) has an owner and happens after the call → "actionItems" only; (2) is an agreement on how to proceed but not itself an assignable task → "whatWasDecided" only; (3) everything else discussed → "whatWasCovered" only.
